...
@section('content')
            ...
            <div style="z-index: 10;" class="border mh-100 m-2 bg-light d-flex flex-column position-absolute overflow-auto">
                        ...
                        <div class="form-check px-3">
                            @if (session()->has('kota'))
                                @if ($item->id == session()->get('kota'))
                                    <input class="kabupaten" type="checkbox" checked="true" value={{ $item->id }}
                                        id={{ $item->id }}>
                                @else
                                    <input class="kabupaten" type="checkbox" value={{ $item->id }}
                                        id={{ $item->id }}>
                                @endif
                            @else
                                <input class="kabupaten" type="checkbox" checked="true" value={{ $item->id }}
                ...
                <div class="inputs border-top py-2">
                    @foreach ($sub_kategori as $item)
                        <div class="form-check px-3">
                            <input class="kategori" type="checkbox" checked="true" value={{ $item->id }}
                                id={{ $item->id }}>
                            <label class="form-check-label" for={{ $item->id }}>
                                @switch($item->nama)
                                    @case('Bangunan')
    ...
    <script>
        ...
        data.forEach(function(item) {
            ...
            if ((item.latitude != null && item.longitude != null)) {
                ...
                const newFeature = {
                    ...
                    "geometry": {
                        "type": "Point",
                        "coordinates": [item.longitude, item
                            .latitude] // Replace with your actual coordinates from the database
                    },
                    "properties": {
                        "nama": item.nama,
        ...
        const geojsonLayer = L.geoJSON(null, {
            filter: (feature) => {
                const isKabChecked = checkboxStates.kabupaten.includes(feature.properties.kabupaten_id
                .toString());
                const isCatChecked = checkboxStates.kategori.includes(feature.properties.sub_kategori_id
                    .toString());
            ...
            for (let input of document.querySelectorAll('input')) {
                if (input.checked) {
                    switch (input.className) {
                        case 'kabupaten':
                            checkboxStates.kabupaten.push(input.value);
                            break
                        case 'kategori':
                            checkboxStates.kategori.push(input.value);
                            break
                        case 'status':
                            checkboxStates.kategori.push(input.value);
                            break
        ...
        function markerOnClick(data) {
            ...
            if (data.link_360.length > 2) {
                link360 = '<a href=' + data.link_360 +
                    ' target="_blank" type="button" class=""><u>Link Website Virtual Tour 360</u></a>';
            }
            ...
            data.fotos.forEach((foto, index) => {
                ...
                fotosHtml += '">' +
                    '<img class="d-block w-100 bd-placeholder-img object-fit-cover" src="storage/foto/' + foto.url +
                    '" alt="Slide" />' +
                    '</div>';
            });
